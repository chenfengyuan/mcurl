...
class RangeDownloader:
    ...
    def __init__(self, filesize: int, req: Request, outq: Queue, chunk_size, filename: str):
        self.outq = outq
        self.req = req
        ...
        if cls == Classification.DOWNLOAD_RANGE:
            ...
            for data in c.iter_chunk():
                if data[0] is True:
                    ...
                    start = data[0]
                    chunk = data[1]
                    logger.debug('%s:chunk downloaded:%s', self.filename, start)
                    self.outq.put((Classification.NEW_DOWNLOADED_CHUNK, start, chunk, q))
                    data = q.get()
                    if data == Classification.NEXT_CHUNK_WANTED:
                        logger.debug('%s:downloading next chunk', self.filename)
                        continue
                    elif data == Classification.CURREN_OR_NEXT_CHUNK_IS_DOWNLOADED:
                        logger.debug('%s:range downloader exiting', self.filename)
                        return
                    else:
                        logger.error("%s:unexpected queue data: %s", self.filename, data)
                        raise self.stop_exc
        elif cls == Classification.NO_RANGE_FOR_NOW:
            logger.debug('%s:no range for now', self.filename)
            time.sleep(self.NO_RANGE_RETRY_TIME)
            return
        else:
            logger.error('%s:unknow classification and payload: %s', self.filename, data)
